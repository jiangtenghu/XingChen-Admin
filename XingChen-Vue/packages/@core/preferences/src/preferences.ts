...
const STORAGE_KEY_VERSION = `${STORAGE_KEY}-version`;
const CURRENT_VERSION = '2.0.0'; // XingChen Admin version
...
class PreferenceManager {
  ...
  private loadPreferences(): Preferences {
    // 检查版本，如果版本不匹配则清理缓存
    const cachedVersion = this.cache?.getItem<string>(STORAGE_KEY_VERSION);
    if (cachedVersion !== CURRENT_VERSION) {
      this.clearCache();
      this.cache?.setItem(STORAGE_KEY_VERSION, CURRENT_VERSION);
